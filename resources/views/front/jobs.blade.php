    ...
    <div class="container">     
        <div class="row">
            <div class="col-6 col-md-10 ">
                <h2>Trouver des emplois</h2>  
            </div>
            <div class="col-6 col-md-2">
                <div class="align-end">
                    <select name="sort" id="sort" class="form-control">
                        <option value="1" {{ (Request::get('sort') == '1') ? 'selected' : '' }}>Les plus récents</option>
                        <option value="0" {{ (Request::get('sort') == '0') ? 'selected' : '' }}>Les plus anciens</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-4 col-lg-3 sidebar mb-4">
                <form action="" name="searchForm" id="searchForm">
                    <div class="card border-0 shadow p-4">
                        <div class="mb-4">
                            <h2>Mots-clés</h2>
                            <input value="{{ Request::get('keyword') }}" type="text" name="keyword" id="keyword" placeholder="Mots-clés" class="form-control">
                        </div>
                        ...
                        <a href="" class="btn btn-secondary mt-3">Réinitialiser</a>
                    </div>
                </form>
            </div>
                ...
                <div class="job_listing_area">                    
                    <div class="job_lists">
                        <div class="row">
                                    ...
                                    <div class="card border-0 p-3 shadow mb-4">
                                        <div class="card-body">
                                            ...
                                            <div class="bg-light p-3 border">
                                                ...
                                                {{-- <p>Mots-clés: {{ $job->keywords }}</p>
                                                <p>Catégorie: {{ $job->category->name }}</p>
                                                <p>Expérience: {{ $job->experience }}</p> --}}
                                            ...
                                            <a href="{{ route('jobDetail',$job->id) }}" class="btn btn-primary btn-lg">Détails</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @else
                            <div class="col-md-12">Aucun emploi trouvé</div>                                
                            @endif                           
                        </div>
                    </div>
                </div>
...
<script>
    $("#searchForm").submit(function(e){
        ...
        // Si les mots-clés ont une valeur
        if (keyword != "") {
            url += '&keyword='+keyword;
        }

        // Si le lieu a une valeur
        if (location != "") {
            url += '&location='+location;
        }

        // Si la catégorie a une valeur
        if (category != "") {
            url += '&category='+category;
        }

        // Si l'expérience a une valeur
        if (experience != "") {
            url += '&experience='+experience;
        }

        // Si l'utilisateur a coché des types d'emploi
